defmodule ExOpenAI.Codegen.ComponentModuleGenerator do
  ...
  def generate_module(%Schema{name: name, type: "object"} = schema) do
    ...
    moduledoc = if schema.description do
      quote do
  ...
  defp generate_type_spec(%Schema{properties: properties, required: required}) when is_map(properties) do
    ...
    |> Enum.map(fn {prop_name, prop_schema} ->
      ...
      type_ast = if prop_name in required_list do
        prop_type
      else
